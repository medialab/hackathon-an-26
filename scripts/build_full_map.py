...
def main() -> None:
    ...
    eqs = _json.loads((ROOT / "data" / "equalitiesNonDeclare.json").read_text())
    ...
    forced = sorted({j for _, j, _ in links})
    ...
    out = {
        "stats": (f"{len(hidx):,} activités HATVP ({'toutes' if len(hidx)==len(hm) else f'sur {len(hm):,}'}) · "
                  ...
                  f"{len(out_links)} liens (exercice antérieur)").replace(",", " "),
        "domains": top + ["Autres"],
        ...
        "a_full": a_full,
        "n_hatvp_total": int(len(hm)), "n_amd": int(len(am)),
    }
